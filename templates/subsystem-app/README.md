...
| **ESG** (External Service Gateway) | An anti-corruption boundary around an external system — a payment provider, a partner API. It normalises inbound webhooks into your events and delivers your outbound events to the external API, so external quirks never leak into the rest of the subsystem. |
...
subsystem.yaml          you edit this — the description of your subsystem
      │
      ▼
pattern library         a pinned Terraform dependency — composes the resources
      │
      ▼
...
2. **A state bucket** — run the **Provision state bucket** GitHub Action (Actions tab → run workflow). Holds Terraform state, encrypted and versioned.
...
**Per environment** (dev / staging / prod — these are [GitHub Environments](https://docs.github.com/en/actions/deployment/targeting-different-environments)):
...
1. **Edit** [`infra/terraform/subsystem.yaml`](infra/terraform/subsystem.yaml) to describe your subsystem (see below). The starter is a single BFF plus the operations set — enough to deploy.
...
4. Merge, then run the **Manage infrastructure** Action → `plan`, review the output.
5. Run it again → `apply` (waits for the environment approval).
...
bffs:
  - name: cart               # a user activity -> its own API + db + functions
    activity: crud
...
The events listed under `publishes` / `subscribes` / `egress` are how blocks talk to each other — a control service `subscribes` to what a BFF `publishes`, an ESG's `egress` matches events you want delivered to an external system. Names are yours to choose; just keep them consistent across the manifest.
...
Deploys run from the **Manage infrastructure** GitHub Action (Actions tab → run workflow). Pick an action and an environment:

- **plan** — preview the changes. Safe; makes nothing.
- **apply** — build and publish the Lambda artefacts, then apply. Waits for the environment's required reviewers before it runs.
- **destroy-plan** / **destroy** — preview / tear down.

State is keyed per environment (`<repo>/<env>.tfstate`), so dev, staging and prod never collide. Authentication is OIDC — no stored AWS credentials.
...
- **`terraform init` can't find the module / a 404 on the library** — the library is referenced by a pinned version (`?ref=v0.2.0`). Make sure that tag exists and your repo can access the library (it's a private dependency).
